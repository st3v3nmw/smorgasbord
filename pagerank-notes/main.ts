import { Plugin, addIcon, iterateCacheRefs, getLinkpath, ItemView } from 'obsidian';
import * as graph from "pagerank.js";
import { DataSet } from "vis-data/esnext";
import { Network } from "vis-network/esnext";

const PAGERANK_VIEW_TYPE = "pagerank-list-view";
...
export default class MyPlugin extends Plugin {
	...
	private graph_view: PagerankGraphView;
	...
	async onload(): void {
		...
		this.addRibbonIcon('crosshairs', 'Recalculate Pageranks', _ => {
			this.recalculate_pageranks();
		});

		this.registerView(
			PAGERANK_VIEW_TYPE,
			(leaf) =>
				(this.graph_view = new PagerankGraphView(
					leaf,
					this
				))
		);
	...
	async recalculate_pageranks(): void {
        ...
        let incoming_links = {},
            outgoing_links = {};
        notes.forEach((file) => {
            incoming_links[file.path] = { };
            outgoing_links[file.path] = { };
        });
			...
			iterateCacheRefs(
				...
				(cb) => {
					...
					if (target != null && target.extension == "md") {
						if (
							!(
								target.path in outgoing_links[source.path]
							)
						)
							outgoing_links[source.path][target.path] = 0;
						outgoing_links[source.path][target.path]++;

						if (
							!(
								target.path in
								incoming_links[target.path]
							)
						)
							incoming_links[target.path][source.path] = 0;
						incoming_links[target.path][source.path]++;
					}
				}
			);
		});
		...
		for (let source in outgoing_links) {
			for (let target in outgoing_links[source])
				graph.link(
					source,
					target,
					outgoing_links[source][target]
				);
		}
	...
	initView(): void {
        if (this.app.workspace.getLeavesOfType(PAGERANK_VIEW_TYPE).length) {
            return;
        }

        this.app.workspace.getRightLeaf(false).setViewState({
            type: PAGERANK_VIEW_TYPE,
            active: true,
        });
    }
}

class PagerankGraphView extends ItemView {
    plugin: MyPlugin;
    ...
    public onHeaderMenu(menu: Menu): void {
        menu.addItem((item) => {
            item.setTitle("Close")
                .setIcon("cross")
                .onClick(() => {
                    this.app.workspace.detachLeavesOfType(PAGERANK_VIEW_TYPE);
                });
        });
    }
    ...
    public redraw(): void {
		...
		let n = [];
		let id = 1;
		for (let label in this.plugin.outgoing_links) {
			n.push({"id": label, value: this.plugin.data.pageranks[label], label: label.split('/').pop()});
			id++;
		}

		let e = [];
		for (let from in this.plugin.outgoing_links) {
			let targets = this.plugin.outgoing_links[from];
			for (let to in targets) {
				e.push({from, to, arrows: "to"});
			}
		}
